...
with open('F:\\吉胡阿川\\01lhjk\\事业\\AXS设计工作室\\AXS_设计工作室客户需求深度调研表_restored.html', 'r', encoding='utf-8') as f:
    text = f.read()

text = text.replace('完成深度探索 · 生成需求报告/button>', '完成深度探索 · 生成需求报告</button>')

bottom_ui = '''
        <div style="display: flex; gap: 20px; margin-top: 40px;">
            <button type="button" class="submit-btn" id="savePageBtn" style="flex: 1; background: #2a2a2a; color: #d4af37; border: 1px solid #d4af37;">1: 保存当前页面</button>
...
new_script = '''
    <section id="resultPanel" class="result-panel" aria-live="polite">
        ...
        <div class="module-title">客户需求报告已生成</div>
        <p class="helper-text">JSON 文件已下载，或者您可以在此复制 Markdown 格式。</p>
        <div class="result-actions">
            <button type="button" class="action-btn" id="copyReport">复制 Markdown</button>
        </div>
